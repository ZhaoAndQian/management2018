...
export default {
  ...
  methods: {
    ...
    createClick: () => {
      _this.form.id = 0
      _this.axios.post('/Role/addone', _this.form).then((response) => {
    ...
    updateClick: () => {
      _this.axios.post('/Role/UpdateList', [_this.form]).then((response) => {
        if (response.data.state === '0') {
    ...
    delClick: () => {
      _this.$confirm('确定要删除吗?', '提示', {
        ...
        _this.axios.delete('/Role/del/' + _this.form.id).then((response) => {
