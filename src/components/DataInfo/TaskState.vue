...
<div>
  ...
  <el-col :span="10">
    ...
    <el-form-item label="名称" prop="stateName">
...
export default {
  ...
  methods: {
    searchClick: () => {
      _this.axios.get('/TaskState/selectall').then((response) => {
        ...
        if (data.state === '0') {
          ...
          _this.$message.error('获取任务状态信息出错：' + data.msg)
        }
      })
    ...
    createClick: () => {
      _this.form.id = 0
      _this.axios.post('/TaskState/addone', _this.form).then((response) => {
    ...
    updateClick: () => {
      _this.axios.post('/TaskState/UpdateList', [_this.form]).then((response) => {
        if (response.data.state === '0') {
    ...
    delClick: () => {
      _this.$confirm('确定要删除吗?', '提示', {
        ...
        _this.axios.delete('/TaskState/del/' + _this.form.id).then((response) => {
  ...
  data () {
    return {
      rules: {
        stateName: [{required: true, message: '请输入任务状态名称', trigger: 'blur'}]
      },
      tableData: [
      ],
      form: {
