...
export default {
  ...
  methods: {
    searchClick: () => {
      _this.axios.get('/EventType/selectall').then((response) => {
        ...
        if (data.state === '0') {
          ...
          _this.$message.error('获取事件类型信息出错：' + data.msg)
        }
      })
    ...
    createClick: () => {
      _this.form.id = 0
      _this.axios.post('/EventType/addone', _this.form).then((response) => {
    ...
    updateClick: () => {
      _this.axios.post('/EventType/UpdateList', [_this.form]).then((response) => {
        if (response.data.state === '0') {
    ...
    delClick: () => {
      _this.$confirm('确定要删除吗?', '提示', {
        ...
        _this.axios.delete('/EventType/del/' + _this.form.id).then((response) => {
  ...
  data () {
    return {
      rules: {
        eventName: [{required: true, message: '请输入事件类型名称', trigger: 'blur'}]
      },
      tableData: [
      ],
      form: {
